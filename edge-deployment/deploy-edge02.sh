#!/bin/bash
# Deploy observability stack to VM-4 (edge02)
# To be run on VM-4 or from VM-1 via SSH

EDGE_NAME="edge02"
EDGE_IP="172.16.4.176"
VM1_IP="172.16.0.78"

echo "========================================="
echo "Deploying Observability Stack to Edge02"
echo "========================================="

# Create edge02-specific configuration
cat > /tmp/edge02-stack.yaml << EOF
# Edge02 Observability Stack
...
---
# 1. Config Sync RootSync for Edge02
apiVersion: configsync.gke.io/v1beta1
kind: RootSync
metadata:
  name: edge02-root-sync
  namespace: config-management-system
spec:
  sourceFormat: unstructured
  sourceType: git
  git:
    repo: http://${VM1_IP}:8888/gitops/edge02-configs
    branch: main
    auth: none
    period: 30s
  sync:
...
echo "Configuration generated at /tmp/edge02-stack.yaml"

# Deploy to edge02
if command -v kubectl &> /dev/null; then
    echo "Deploying to local Kubernetes cluster..."
    kubectl apply -f /tmp/edge02-stack.yaml
    ...
    echo "kubectl not found. Please install Kubernetes first or copy /tmp/edge02-stack.yaml to edge02"
    echo ""
    echo "To deploy manually on edge02:"
    echo "1. Copy /tmp/edge02-stack.yaml to edge02"
    echo "2. SSH to edge02: ssh ubuntu@${EDGE_IP}"
    echo "3. Apply configuration: kubectl apply -f edge02-stack.yaml"
fi

echo ""
echo "========================================="
echo "Edge02 Deployment Summary:"
echo "========================================="
...
echo "  On VM-1: curl http://${VM1_IP}:8428/api/v1/label/__name__/values | grep edge02"
echo "========================================="