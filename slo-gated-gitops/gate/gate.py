...
def parse_slo_string(slo_string: str) -> List[Dict[str, Union[str, float]]]:
    ...
    for constraint in slo_string.split(','):
        ...
        try:
            ...
            raise ValueError(f"Invalid SLO format: threshold must be numeric in {constraint}")
...
def validate_metrics_against_slos(metrics: Dict[str, float], slos: List[Dict[str, Union[str, float]]]) -> bool:
    """
...
def fetch_metrics(url: str) -> Dict[str, float]:
    ...
    try:
        ...
        logger.info('Metrics fetched successfully', extra={
            'url': url,
            'metrics': metrics,
...
def main() -> int:
    ...
    logger.info('SLO Gate starting', extra={
        'slo_string': args.slo,
    ...
    try:
        ...
        logger.info('SLO validation PASSED', extra={
        ...
        logger.error('SLO parsing error', extra={
            'error': str(e),
            'slo_string': args.slo
        })
        return 1
        ...
        logger.error('Metrics fetch failed', extra={
            'error': str(e),
            'url': args.url
        })
        return 1
        ...
        duration_ms = (datetime.utcnow() - start_time).total_seconds() * 1000
        logger.error('SLO validation FAILED', extra={
        ...
        logger.error('Missing required metric', extra={
            'error': str(e),
            'available_metrics': list(metrics.keys()) if 'metrics' in locals() else []
        })
        return 1
