...
class TestMetricsConfig:
    ...
    def test_metrics_config_customization(self):
        ...
        config = MetricsConfig(
            latency_p95_ms=25.5, success_rate=0.999, throughput_p95_mbps=150.0
        )
    ...
    def test_metrics_logging_json_format(self, client, caplog):
        ...
        assert (
            len(metrics_records) > 0
        ), "Expected at least one log record with metrics data"

        # Verify the log record has the expected structure
        metrics_record = metrics_records[0]
        assert hasattr(
            metrics_record, "metrics"
        ), "Log record should have metrics attribute"
        assert hasattr(
            metrics_record, "duration_ms"
        ), "Log record should have duration_ms attribute"
        assert isinstance(
            metrics_record.metrics, dict
        ), "Metrics should be a dictionary"
        assert (
            "latency_p95_ms" in metrics_record.metrics
        ), "Metrics should contain latency_p95_ms"
        assert (
            "success_rate" in metrics_record.metrics
        ), "Metrics should contain success_rate"
        assert (
            "throughput_p95_mbps" in metrics_record.metrics
        ), "Metrics should contain throughput_p95_mbps"
...
class TestAdapterConfiguration:
    ...
    def test_adapter_uses_env_example_pattern(self):
        ...
        with patch.dict(
            os.environ,
            {"ADAPTER_PORT": "8080", "ADAPTER_HOST": "0.0.0.0", "LOG_LEVEL": "INFO"},
        ):
            app = create_app()
