...
class TestSLOStringParsing:
    ...
    def test_parse_simple_slo_string(self):
        ...
        expected = [{"metric": "latency_p95_ms", "operator": "<=", "threshold": 15.0}]
...
class TestCLIIntegration:
    ...
    def test_cli_help_option(self):
        ...
        result = subprocess.run(
            [
                sys.executable,
                "/home/ubuntu/nephio-intent-to-o2-demo/slo-gated-gitops/gate/gate.py",
                "--help",
            ],
            capture_output=True,
            text=True,
...
class TestJSONLogging:
    ...
    @patch("gate.gate.validate_metrics_against_slos")
    @patch("gate.gate.parse_slo_string")
    def test_json_logging_on_success(
        self, mock_parse, mock_validate, mock_fetch, caplog
    ):
        ...
        assert any(
            "SLO Gate starting" in msg for msg in messages
        ), "Expected starting message"
        assert any(
            "SLO validation PASSED" in msg for msg in messages
        ), "Expected success message"

    @patch("gate.gate.fetch_metrics")
    @patch("gate.gate.validate_metrics_against_slos")
    @patch("gate.gate.parse_slo_string")
    def test_json_logging_on_failure(
        self, mock_parse, mock_validate, mock_fetch, caplog
    ):
        ...
        assert any(
            "SLO validation FAILED" in msg for msg in messages
